...
public class TodayReportServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		String sqlCounts = "SELECT event_type, COUNT(*) AS cnt " + "FROM activelog " + "WHERE user_id = ? "
				+ "  AND DATE(event_time) = CURDATE() " + "  AND event_type <> '테스트' "
				+ "  AND event_type NOT LIKE '기분:%' " + "GROUP BY event_type";

		// ✅ 오늘 기분(마지막 1개)
		String sqlMood = "SELECT event_type " + "FROM activelog " + "WHERE user_id = ? "
				+ "  AND DATE(event_time) = CURDATE() " + "  AND event_type LIKE '기분:%' " + "ORDER BY event_time DESC "
				+ "LIMIT 1";
