...
public class NotificationService {
	...
	public List<NotifDTO> build(Connection con, String userId, long sinceMs) throws Exception {
		...
		if (wakeTime != null) {
			...
			addTwoStage(plans, "기상_약", 1, base, "기상 약", "기상 " + wakeMedBefore + "분 전이에요", "기상 약", "아직 기상 약 기록이 없어요");
		}

		if (sleepTime != null) {
			LocalTime base = sleepTime.minusMinutes(sleepMedBefore);
			addTwoStage(plans, "취침_약", 1, base, "취침 약", "취침 " + sleepMedBefore + "분 전이에요", "취침 약", "아직 취침 약 기록이 없어요");
		}
		...
		if (preMin != null) {
			if (breakfast != null)
				addTwoStage(plans, "식전_약", 1, breakfast.minusMinutes(preMin), "식전 약",
						"아침 식사 " + preMin + "분 전이에요. " + "약먹을 시간입니다!", "식전 약", "아직 식전 약 기록이 없어요");

			if (lunch != null)
				addTwoStage(plans, "식전_약", 2, lunch.minusMinutes(preMin), "식전 약",
						"점심 식사 " + preMin + "분 전이에요. " + "약먹을 시간입니다!", "식전 약", "아직 식전 약 기록이 없어요");

			if (dinner != null)
				addTwoStage(plans, "식전_약", 3, dinner.minusMinutes(preMin), "식전 약",
						"저녁 식사 " + preMin + "분 전이에요. " + "약먹을 시간입니다!", "식전 약", "아직 식전 약 기록이 없어요");
		}
		...
		if (postMin != null) {
			if (breakfast != null)
				addTwoStage(plans, "식후_약", 1, breakfast.plusMinutes(postMin), "식후 약", "아침 식사 " + postMin + "분 후예요",
						"식후 약", "아직 식후 약 기록이 없어요");

			if (lunch != null)
				addTwoStage(plans, "식후_약", 2, lunch.plusMinutes(postMin), "식후 약", "점심 식사 " + postMin + "분 후예요", "식후 약",
						"아직 식후 약 기록이 없어요");

			if (dinner != null)
				addTwoStage(plans, "식후_약", 3, dinner.plusMinutes(postMin), "식후 약", "저녁 식사 " + postMin + "분 후예요", "식후 약",
						"아직 식후 약 기록이 없어요");
		}
		...
		for (AlarmPlanDTO p : plans) {
			...
			if (now.isBefore(fire.minusMinutes(WINDOW_MIN)) || now.isAfter(fire.plusMinutes(WINDOW_MIN)))
				continue;
			...
			if ("LATE".equals(p.getStage())) {
				...
				boolean done = activeLogDAO.existsEventBetween(con, userId, eventType, p.getBaseTime(),
						p.getBaseTime().plusMinutes(LATE_AFTER_MIN));
				if (done)
					continue;
			}

			String key = "ROU_" + userId + "_" + todayKey(today) + "_" + p.getRoutineType() + "_" + p.getSeq() + "_"
					+ p.getStage();
	...
	private void addTwoStage(List<AlarmPlanDTO> plans, String routineType, int seq, LocalTime baseTime, String titleOn,
			String bodyOn, String titleLate, String bodyLate) {
		if (baseTime == null)
			return;
		plans.add(new AlarmPlanDTO(routineType, seq, baseTime, baseTime, "ONTIME", titleOn, bodyOn));
		plans.add(new AlarmPlanDTO(routineType, seq, baseTime, baseTime.plusMinutes(LATE_AFTER_MIN), "LATE", titleLate,
				bodyLate));
	}
